/**
 * CastPlay README 渲染截图脚本（readme-craft v2.2 T18）
 *
 * CastPlay 是 FastAPI + React + Android 数字标牌，无统一 Web 入口可截图。
 * 统一采用"截 GitHub README 渲染快照"兜底方案：
 *   1. 打开 GitHub 仓库 README 渲染页
 *   2. 等 README 区域加载完
 *   3. 截 README 区域 + 桌面 / 移动视口
 *   4. 保存到 assets/screenshots/
 *
 * v3.1 升级路线：未来可加 FastAPI Swagger UI（/docs）和 React admin 登录页截图。
 *
 * 使用方法：
 *   1. npm install
...
const REPO_URL = 'https://github.com/davyzhong/CastPlay';
...
async function main() {
  console.log(`📸 CastPlay README 渲染截图（readme-craft v2.2 T18）`);
  ...
  console.log(`💡 v3.1 升级计划：补 FastAPI /docs Swagger + React admin 登录页截图`);
